...
def load_EOD_data(data_path, market_name, tickers, steps=1):
    ...
    for index, ticker in enumerate(tickers):
        ...
        if index == 0:
            ...
            eod_data = np.zeros(
                [len(tickers), single_EOD.shape[0], single_EOD.shape[1] - 1],
                dtype=np.float32,
            )
            masks = np.ones([len(tickers), single_EOD.shape[0]], dtype=np.float32)
            ground_truth = np.zeros(
                [len(tickers), single_EOD.shape[0]], dtype=np.float32
            )
...
def load_graph_relation_data(relation_file, lap=False):
    ...
    mask_flags = np.equal(
        np.zeros(rel_shape, dtype=int), np.sum(relation_encoding, axis=2)
    )
    ajacent = np.where(
        mask_flags, np.zeros(rel_shape, dtype=float), np.ones(rel_shape, dtype=float)
    )
...
def load_relation_data(relation_file):
    relation_encoding = np.load(
        relation_file
    )  # N ×N ×K, pairwise relationship for K type
    print("relation encoding shape:", relation_encoding.shape)
    rel_shape = [relation_encoding.shape[0], relation_encoding.shape[1]]
    mask_flags = np.equal(
        np.zeros(rel_shape, dtype=int), np.sum(relation_encoding, axis=2)
    )  # 0 - entity has at least one relationship, 1 - no relationship
    mask = np.where(
        mask_flags, np.ones(rel_shape) * -1e9, np.zeros(rel_shape)
